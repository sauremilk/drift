...
## 2026-04-24 - Signal Dependency DAG + Context-Aware Priority (v2.41.0)

- Risk ID: RISK-DAG-2026-04-24-001
- Component: `src/drift/signals/dependency_dag.py`, `src/drift/finding_priority.py`, `src/drift/scoring/engine.py`
- Type: New module + behavioral change — topological signal scheduling + context-aware finding priority
- Description: `dependency_dag.py` introduces a Kahn-algorithm topological sorter for signal class ordering based on `depends_on_signals`. `finding_priority.py` enriches priority using signal metadata weight, stack depth, and file change frequency. `scoring/engine.py` tracks degradation causes per-signal.
- Trigger: All drift analyze runs; ordering and priority are always active.
- Impact: Finding deduplication increased (63->60 findings in golden snapshot). Priority ordering may reorder findings in agent-driven fix-plans.
- Mitigation:
  - DAG scheduler uses stable tiebreak (class name) for deterministic output.
  - Priority enrichment is additive; existing fingerprints and signal logic unchanged.
  - Golden snapshot updated to reflect new deduplication baseline.
  - 207/207 precision-recall tests passing after change.
- Verification:
  - `.venv\Scripts\python.exe -m pytest tests/test_signal_dependency_dag.py tests/test_issue_370_context_aware_priority.py tests/test_golden_snapshot.py -q --tb=short` (all passed).
- Residual risk: Low. Topological cycle detection raises ValueError if a cycle is introduced; stable tiebreak prevents non-determinism.
