...
- Input path changes: Retrieval liest ausschliesslich lokale Repo-Dateien (`POLICY.md`, `ROADMAP.md`, `decisions/ADR-*.md`, `audit_results/*.md`, `src/drift/signals/*.py`, `benchmark_results/v*_feature_evidence.json`). Keine Netzwerk-Calls, kein Zugriff auf `.git`-Historie, kein externer Index-Pull. MCP-Tools akzeptieren `path` (Repo-Root) + Query-Parameter.
...
- STRIDE review:
  ...
  - **R (Repudiation): Improves.** Jede Agenten-Antwort, die `fact_id` + `sha256` zitiert, wird verifizierbar. Ein Agent, der später behauptet, etwas nicht gesagt zu haben, kann durch Fact-ID-Lookup widerlegt werden (solange Corpus-History nachvollziehbar bleibt). Append-Only `decisions/fact_id_migrations.jsonl` macht ID-Remappings auditierbar.
