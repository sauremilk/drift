...
def _is_generated_file_path(path: str) -> bool:
    """Return True for generated-file path conventions.

    TVS should focus on actionable production volatility, not expected churn
    from code generation outputs.
    """
...
def _has_generated_header(path: Path) -> bool:
    ...
    try:
        with path.open("r", encoding="utf-8", errors="replace") as handle:
            # Code generators place these markers at the top of files; keep the
            # scan window intentionally small to avoid unnecessary I/O.
            header = "".join(next(handle, "") for _ in range(_GENERATED_HEADER_MAX_LINES)).lower()
    except OSError:
        return False
...
class TemporalVolatilitySignal(BaseSignal):
    ...
    def analyze(
        ...
        for history in histories:
            history_path = history.path.as_posix()

            if _is_test_file_path(history_path):
                continue
            if _is_generated_file_path(history_path):
                continue
            ...
            if max_z < z_threshold:
                continue
            if _has_generated_header(history.path):
                continue
