...
# ---------------------------------------------------------------------------
# Repair skill renderer (generated by Skill Synthesizer)
# ---------------------------------------------------------------------------


def render_repair_skill_md(draft: dict) -> str:
    """Render a *repair* skill draft to SKILL.md text.

    Unlike guard skills (from ``SkillBriefing``), repair skills are produced
    by the Skill Synthesizer and carry fix patterns and negative examples.

    Parameters
    ----------
    draft:
        A ``SkillDraft.to_dict()`` result with kind == 'repair'.
