"""Negative-context generators — one per signal type.

Each generator transforms a :class:`~drift.models.Finding` into one or more
:class:`~drift.models.NegativeContext` items that describe the anti-pattern
...
def _gen_ecm(finding: Finding) -> list[NegativeContext]:
    ...
    exc_str = ", ".join(str(e) for e in exception_types[:5]) if exception_types else "mixed types"
    ...
    if comparison_ref:
        desc_parts.append(f"Contract changed relative to {comparison_ref}.")
...
def _gen_avs(finding: Finding) -> list[NegativeContext]:
    ...
    canonical_parts.append("# Use dependency injection or interfaces for cross-layer access")
...
def _gen_ccc(finding: Finding) -> list[NegativeContext]:
    ...
    file_a = meta.get("file_a") or (finding.file_path.as_posix() if finding.file_path else "file_a")
    ...
    if co_change_weight is not None:
        desc_parts.append(
            f"Co-change strength: {co_change_weight:.1f} (confidence {confidence_val:.0%})."
        )
    desc_parts.append("When changing one file, you MUST review and update the others.")
...
def _gen_isd(finding: Finding) -> list[NegativeContext]:
    ...
    return [
        NegativeContext(
            ...
            confidence=0.9,
            rationale=(
                "AI agents copy dev configuration as defaults (FMEA FM-16, RPN 180, CWE-1188)."
            ),
...
def _gen_nbv(finding: Finding) -> list[NegativeContext]:
    ...
    return [
        NegativeContext(
            ...
            scope=_scope_from_finding(finding),
            description=(
                f"Naming violation: {violation}. Follow this repository's naming conventions."
            ),
            forbidden_pattern=(
...
def _gen_gcd(finding: Finding) -> list[NegativeContext]:
    ...
    return [
        NegativeContext(
            ...
            severity=finding.severity,
            scope=_scope_from_finding(finding),
            description=(f"'{func_name}' uses deep nesting instead of early returns."),
            forbidden_pattern=(
...
def _gen_dca(finding: Finding) -> list[NegativeContext]:
    ...
    return [
        NegativeContext(
            ...
            confidence=0.75,
            rationale=(
                "AI agents add 'just in case' code that is never used (FMEA FM-10, RPN 96)."
            ),
        )
    ]
...
def _gen_cir(finding: Finding) -> list[NegativeContext]:
    ...
    return [
        NegativeContext(
            ...
            affected_files=_affected(finding),
            confidence=0.9,
            rationale=("AI agents cannot see the import graph (FMEA FM-13, RPN 96)."),
        )
    ]
...
def _gen_foe(finding: Finding) -> list[NegativeContext]:
    ...
    return [
        NegativeContext(
            ...
            confidence=0.75,
            rationale=(
                "AI agents accumulate dependencies without cohesion awareness (FMEA FM-15, RPN 96)."
            ),
        )
    ]
...
def _gen_tvs(finding: Finding) -> list[NegativeContext]:
    ...
    meta = finding.metadata
    module = meta.get("module") or (finding.file_path.as_posix() if finding.file_path else "module")
    ...
    return [
        NegativeContext(
            ...
            canonical_alternative=(
                ...
                "# Prefer adapting this module to the dominant project behavior "
