...
- **Signal exception stack traces swallowed at WARNING log level (#411)**: `SignalPhase.run()` in `pipeline.py` and `IncrementalSignalRunner.run()` in `incremental.py` now pass `exc_info=True` unconditionally to the `logger.warning()` call on signal failure, instead of `exc_info=logger.isEnabledFor(logging.DEBUG)`. Stack traces are now always attached to the warning record without requiring `DEBUG` log level.
- **`save_baseline()` non-atomic write and corrupt-file crashes in callers (#413)**: `save_baseline()` in `baseline.py` now uses `tempfile.mkstemp` + `Path.replace()` (atomic rename) instead of `write_text()`, eliminating partial-file corruption on SIGKILL. The three unsafe callers (`ci.py`, `_shared.py`, `baseline.py`) now catch `(OSError, ValueError, json.JSONDecodeError)` and emit a user-friendly error message instead of a bare traceback.
- **MCP tools fail at runtime with raw `No module named 'drift.X'` errors (#365)**: `_run_api_tool` in `mcp_utils.py` now detects `ImportError` for internal `drift.*` modules and returns a user-friendly `DRIFT-5001` response with actionable `agent_instruction` (reinstall via `pip install --upgrade 'drift-analyzer[mcp]'`) instead of the confusing raw exception string. The MCP `--serve` startup also runs `_check_mcp_core_imports()` and raises `DRIFT-2011` immediately if core modules are broken, rather than failing on first tool invocation. New error code `DRIFT-2011` added to the error registry.
