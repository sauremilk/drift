...
- **Scan now warns when active signal filters match no registered signal (#522)**: `SignalPhase.run()` now records a machine-readable degradation event (`no_signals_matched`) when `active_signals` yields an empty execution set, and `api.scan()` surfaces this as a user-facing warning including the requested signal IDs. This prevents silent "0 findings" outcomes when filters are invalid or unmatched.
- **File discovery now includes modern module/stub extensions (#521)**: `discover_files()` and `detect_language()` now recognize `.mjs`, `.cjs`, `.mts`, `.cts`, and `.pyi` so these files are no longer silently omitted from default analysis scope. Regression tests lock both language detection and default-include coverage.
- **Expired inline suppressions now auto-reactivate findings during normal analysis (#514)**: `filter_findings` now evaluates `until:YYYY-MM-DD` metadata and skips suppressions whose date is in the past, so previously muted findings re-surface automatically in `analyze`/`check`/`scan`. Pipeline results now carry expired suppression locations in analysis metadata, and rich output shows a warning when expired suppressions were encountered.
- **TypeScript/JavaScript skip visibility restored when tree-sitter is unavailable (#516)**: `discover_files()` now always considers `*.ts`, `*.tsx`, `*.js`, and `*.jsx` during default discovery when TypeScript analysis is enabled, so unsupported-language files are counted in `skipped_languages` instead of being silently omitted. Discovery cache entries now persist and replay `skipped_languages` metadata on cache hits. JSON output now also exposes `analysis_status.skipped_languages`, and rich output coverage is locked by regression tests.
...
- **MCP `drift_session_start` non-autopilot response enrichment (#495)**: `run_session_start()` in `mcp_router_session` now enriches the returned payload with `_enrich_response_with_session(...)` for manual sessions (`autopilot=false`) as well, so agent workflows always receive the top-level `session` block (`session_id`, `phase`, `next_tools`, `tasks_remaining`) consistently across both session-start modes; a regression test locks this contract.
- **SessionManager bounded session pool for MCP stability (#494)**: `SessionManager.create()` now enforces a configurable maximum active-session cap (default 50) and emits a capacity warning when utilization reaches the configured threshold ratio (default 80%). When the pool is exhausted, `drift_session_start` now returns a structured recoverable `DRIFT-4000` error response instead of allowing unbounded session growth.
