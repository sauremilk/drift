...
- **Fingerprint v2 (symbol-based, line-independent) + fuzzy HEAD-subtraction (ADR-082, ADR-083, #531).** Closes the last-mile false-positive path in `drift_diff`: pre-existing HEAD findings that merely shifted lines or whose title carried a volatile metric were reported as "new" after every agent-driven edit. Field-test 2026-04-21 (`work_artifacts/reduce_findings_2026-04-21/`) showed ~6 of 13 "new" findings were pure line-shift false positives; post-fix PFS findings accounted for the rest.
  - **ADR-082 — Fingerprint v2.** `finding_fingerprint()` now hashes `(signal_type, file_path, symbol_identity, stable_title)` instead of `(signal_type, file, start_line, end_line, title)`. `symbol_identity` uses the cascade `logical_location.fully_qualified_name → logical_location.name → symbol → ""`. `stable_title` normalises titles by replacing `\d+` with `<N>` and stripping trailing `(file:line)` parenthesis. Line numbers no longer enter the hash — a function that shifts from line 10 to line 120 keeps the same fingerprint. The v1 helper `finding_fingerprint_v1()` is retained for migration and regression testing. New public helper `stable_title()` is exported for downstream consumers.
  - **Baseline schema v2 with alias.** `_BASELINE_VERSION` bumped to 2. `save_baseline` writes each entry with canonical `fingerprint` (v2), `fingerprint_v1` alias for two minor-release cycles, and a new `symbol` field for debugging. `load_baseline` returns the stored canonical fingerprint set and emits a schema-upgrade warning on v1 files. `baseline_diff` performs a v1+v2 dual-lookup per finding so existing v1 baseline files continue to match without a forced regeneration.
  - **Fuzzy HEAD-subtraction safety net in `drift_diff`.** New `_HeadMatchIndex` dataclass and `get_head_match_index_for_diff()` in `drift.analyzer` compute both the exact-v2 fingerprint set and a fuzzy-key set `(signal, file, stable_title)` from a single HEAD-state analysis. `drift.api.diff._subtract_pre_existing_head` now first checks v2-fingerprint exact match, then falls back to fuzzy-key match for symbol-less findings (TPD, DRS). Default on; disable via `thresholds.diff_fuzzy_head_subtraction: false`. The legacy `get_head_fingerprints_for_diff` public function is kept as a thin wrapper for backward compatibility.
  - **ADR-083 — Pre-Edit Pattern-Scan.** `.github/prompts/drift-fix-loop.prompt.md` now makes `drift_steer(target=<file>)` a **mandatory pre-edit step** whenever a task introduces a new symbol (helper extraction, duplicate consolidation, skeleton function). Agents must extract `patterns_used_in_scope` and adopt the dominant dialect (error-handling style, return-shape) to prevent post-fix PFS findings. Exception applies only for pure local bugfixes without new symbols.
  - **New tests.** `tests/test_baseline.py::TestFingerprintV2Stability` (7 tests: line-shift, metric, trailing-refs, rename, file-move, signal-change, fallbacks), `TestStableTitle` (5 tests), `test_save_writes_v2_schema_with_v1_alias`, `test_v1_schema_baseline_still_loads`, `tests/test_scan_diversity.py::test_uncommitted_ignores_shifted_findings_via_v2_fingerprint`, `test_uncommitted_fuzzy_pass_catches_symbol_less_findings`.
  - **Breaking note.** Externally cached `finding_id` strings (CLI JSON / SARIF) change because they derive from the v2 hash. External consumers have two minor-release cycles to migrate; the baseline schema preserves both v1 and v2 via the `fingerprint_v1` alias in the meantime.

- **Concurrent-writer advisory, replan counter, and resume fix_apply routing (Q3/Q4/Q5, ADR-081 Nachschärfung).** Closes the last three ADR-081 follow-up gaps.
  - Q3 — New `drift.session_writer_lock` module places a best-effort advisory lockfile at `<repo>/.drift-cache/queue.lock` carrying `{pid, session_id, started_at}`. `drift_session_start` probes the existing holder (POSIX `os.kill(pid, 0)` / Windows `OpenProcess`+`GetExitCodeProcess`) and surfaces `concurrent_sessions_detected: bool` plus `concurrent_writer: {pid, session_id, started_at, age_seconds, pid_alive} | null` in the response. When a live writer is detected, `agent_instruction` gets an appended warning so operators can pause the other session. Acquisition unconditionally overwrites ("last session wins") so a crashed session cannot block a restart. `drift_session_end` releases the lockfile when `session_id` matches. Pure stdlib, same cooperative single-writer contract as ADR-081 original — no hard-block, no new blocking path.
  - Q4 — ADR-081 now documents the *latest-plan-wins* replay semantics explicitly: when the event log contains multiple `plan_created` events, the reducer uses only the most recent one and terminal events preceding it are discarded on purpose (task_ids are not globally stable; a replan implies new priorities, not a merge). The `drift_session_start` response adds `resumed_older_plans_discarded: int` as an informational counter so external reviewers can see the replan happened and quantify it.
  - Q5 — `drift_session_start` now rewrites `next_tool_call` to `drift_fix_apply` with the first pending `task_id` when replay restores pending tasks and the plan is not stale (sort key: `priority_score` DESC, original-index ASC tiebreak). `fallback_tool_call` points to `drift_fix_plan`. Stale-plan override (Q2) still takes precedence. Response gains `resumed_next_task_id: str | null`. Closes the adoption gap where agents followed `next_tool_call` to `drift_scan` after a resume and never worked the persisted queue.

- **Plan-staleness surfacing in `drift_session_start` (Q2, ADR-081 Nachschärfung).** Queue-log replay now exposes the age of the restored plan so agents can distinguish a fresh session-restart resume from a zombie plan left behind by an abandoned project. `ReplayedState` carries `plan_created_at` and `plan_session_id`; the `drift_session_start` response adds `resumed_plan_created_at`, `resumed_plan_age_seconds`, and `resumed_plan_stale`. Default staleness threshold is 24 h, overridable via `DRIFT_QUEUE_STALE_SECONDS` (invalid or non-positive values fall back to the default). When `resumed_plan_stale=true`, `agent_instruction` reports the age in hours and recommends `drift_fix_plan`; `next_tool_call` is redirected from `drift_scan` to `drift_fix_plan` so the agent re-prioritises against the current codebase. `fresh_start=true` still suppresses replay entirely.
...
- **`drift_nudge` cold-start latency reduced by ~3.5 s (ADR-085).** `_create_baseline()` previously made three separate I/O passes over the file list: a standalone `discover_files()`, then `analyze_repo()` (which internally discovers + hashes + parses all files), then a redundant third pass re-reading every file for hashes and re-reading every parse-cache JSON. The third pass — ~2000 redundant I/O operations on a 1000-file repo — is now eliminated. `file_hashes` computed inside `IngestionPhase.run()` are surfaced to callers via a new optional `file_hashes_out: dict[str, str] | None` parameter on `AnalysisPipeline.run()`, `_run_pipeline()`, and `analyze_repo()`. `_create_baseline()` passes this out-dict and no longer runs the post-analysis file loop. `parse_map` is set to `{}` (same as the existing disk warm-load path — `IncrementalSignalRunner` handles this correctly). Before: ~4.7 s cold start. After: ~1.2–1.5 s (analyze_repo only). New tests in `tests/test_nudge_cold_start.py`.
