...
- **Automation-Hardening für Pakete 2A/2B/2C (ADR-096).** Zusätzliche Produktivitäts-/Automation-Layer zu den drei QA-2026-Paketen, rückwärtskompatibel: **2A** neuer Subcommand `drift baseline status --format rich|json` (read-only, exit 0 unabhängig von Drift-Menge; JSON-Payload `baseline_exists/baseline_path/baseline_findings/total_findings/known_findings/new_findings/drift_score`). **2B** `actions/setup-python@v5` mit `cache: pip` im Approval-Gate-Workflow (Ersparnis typisch 20–40 s pro PR), zweiter Trigger `workflow_call:` mit Input `approval-label` (downstream-Repos können den Gate via `uses:` ohne Fork einbinden), Report in `$GITHUB_STEP_SUMMARY` ohne PR-Kommentar (konsistent mit "kein ungefragtes Posten"). **2C** neuer Flag `--max-issues N` (default 10, Flood-Guard, Findings jenseits der Cap werden mit `::warning::` übersprungen, exit 0), Dedup iteriert jetzt über **alle** konfigurierten Labels und dedupliziert über Issue-Nummer — Label-Rename bricht Dedup nicht mehr; Summary enthält neuen `capped=N`-Counter. Tests: `tests/test_baseline_status.py` (4) + `tests/test_automation_enhancements.py` (7) — 11 pass.

- **Opt-in Issue-Auto-Filing (Paket 2C, ADR-095).** Die drift GitHub Action bekommt zwei neue Inputs `create-issue` (default `false`) und `issue-labels` (default `drift,agent-block`). Bei `create-issue: true` filt ein neuer Step am Ende der Action für jedes BLOCK-Finding (Severity `critical`/`high`) ein GitHub-Issue in das Consumer-Repo. Dedup: Issue-Body enthält stabilen HTML-Marker `<!-- drift-finding-id: ... -->`, vorhandene offene Issues mit diesem Marker werden übersprungen (Re-Runs spammen nicht). Neues `scripts/gh_issue_dedup.py` ist eigenständig aufrufbar (`--dry-run` für lokale Tests); exit 0 clean, 1 bei gh-Fehlern, 2 bei Usage/Report-Fehler. Neues Issue-Template `.github/ISSUE_TEMPLATE/drift-block.yml` für menschlichen Fallback mit demselben Marker. Opt-in-Default respektiert "kein ungefragtes Posten"-Contract. Tests: `tests/test_gh_issue_dedup.py` (12 pass) + `tests/test_action_yml_paket_2c.py` (6 pass). Audit-Artefakte FMEA und Risk-Register aktualisiert.

- **Human-Approval-Gate Workflow (Paket 2B, ADR-094).** Neuer GitHub-Actions-Workflow `.github/workflows/drift-agent-gate.yml` triggert auf `pull_request`-Events gegen `main` (Pfad-Filter: `src/**`, `tests/**`, Agent-Prompt, Schema-Dateien, `decisions/**`). Parst JSON-Output von `drift analyze` auf BLOCK-Actions (`agent_telemetry.agent_actions_taken[*].gate == "BLOCK"`) und Severity-Findings (`critical`/`high`); failt mit Exit 1, wenn BLOCK-Signal vorhanden und PR-Label `drift/approved` nicht gesetzt ist. Token-Permissions bewusst read-only (`contents:read`, `pull-requests:read`) — Workflow kann sich nicht selbst freigeben. Letzter Step ruft `scripts/verify_gate_not_bypassed.py --all-artifacts` (Soft-Fail bei Exit 2) als Tampering-Check. `.github/CODEOWNERS` um Section "Agent-critical files" erweitert (drift.agent.prompt.md, Schemas, `signal_registry.py`, `intent/handoff.py`, Workflow selbst, `scripts/verify_gate_not_bypassed.py`, `decisions/`) — Maintainer-Review ist damit für alle Agent-Steuer-Dateien Pflicht. Tests: `tests/test_drift_agent_gate_workflow.py` (13 pass) sichern Trigger-Contract, Permissions, CODEOWNERS-Abdeckung und ADR-Status. Audit-Artefakte FMEA, Risk-Register und STRIDE aktualisiert (Tampering + Elevation-of-Privilege).
