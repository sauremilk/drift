...
class TestTypeSafetyBypassSignal:
    ...
    def test_issue_282_fixture_test_support_double_casts_are_treated_as_test_context(
        self, tmp_path: Path
    ) -> None:
        from drift.config import DriftConfig
        from drift.models import ParseResult, Severity
        ...
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(
            "type NativeCommandTestParams = { bot: unknown };\n"
            "type RuntimeEnv = { runtime: unknown };\n"
            "const deps = {\n"
            "  bot: {} as unknown as NativeCommandTestParams['bot'],\n"
    ...
    def test_issue_292_mock_harness_double_casts_are_treated_as_test_context(
        self, tmp_path: Path
    ) -> None:
        from drift.config import DriftConfig
        from drift.models import ParseResult, Severity
        ...
        file_path = tmp_path / "src" / "commands" / "channels.mock-harness.ts"
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(
    ...
    def test_issue_293_status_test_support_double_casts_are_treated_as_test_context(
        ...
        file_path = tmp_path / "src" / "commands" / "status.test-support.ts"
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(
            "const summary = {} as unknown as StatusSummary;\n"
            "const snapshot = {} as unknown as MemoryStatusSnapshot;\n",
            encoding="utf-8",
        )
    ...
    def test_issue_297_menu_test_support_double_casts_are_treated_as_test_context(
        self, tmp_path: Path
    ) -> None:
        from drift.config import DriftConfig
        from drift.models import ParseResult, Severity
        ...
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(
            "type RegisterTelegramNativeCommandsParams = { bot: unknown };\n"
            "const deps = {\n"
            "  bot: {} as unknown as RegisterTelegramNativeCommandsParams['bot'],\n"
            "};\n",
            encoding="utf-8",
        )
        ...
        assert finding.severity == Severity.LOW
        assert finding.metadata.get("finding_context") == "test"
        assert finding.metadata["kind_distribution"].get("double_cast", 0) == 1

    def test_issue_299_provider_usage_test_support_double_casts_are_treated_as_test_context(
        self, tmp_path: Path
    ) -> None:
        from drift.config import DriftConfig
        from drift.models import ParseResult, Severity
        ...
        file_path = tmp_path / "src" / "infra" / "provider-usage.test-support.ts"
        file_path.parent.mkdir(parents=True, exist_ok=True)
        file_path.write_text(
            "const provider = {\n"
